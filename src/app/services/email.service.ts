...
// to be removed
  getEmailStatus(): BehaviorSubject<any[]> {
    ...
    this.http.get<{status_message: string,
      ...
      sent: number,
      errors: number,
      total:number}>(`${this.apiUrl3}/api/status-send-bills`)
      .subscribe((data) => {
        answer = []
...
// to be removed
    ...
    if (!this.groupDone || !this.emailDone) {
      this.subscriptionCheckStatus = this.http2.get<{message: string, status: number, data: any[]}>(`${this.apiUrl3}/api/progressing`).subscribe((statusResponse) => {
  ...
  async sendNewStudentData(studentData: Student[]) {
    ...
    await this.sendNewStudentData2(studentData)
      ...
      .then((data) => {
        ...
        this.responseEmail.next(
          {emailSentStatus: [data.data[0],data.data[1]], emailGroupStatus:[data.data[2],data.data[3]], emailDone: true, groupDone: true}
        )
  ...
  async sendNewStudentData2(studentData: Student[]) {
    return  this.http.post<{message: string, status: number, data: any[]}>(`${this.apiUrl3}/api/actions_gen_email`, studentData)
      .toPromise()
      .then();
  }
  ...
  sendNewStudentDataForGroup2(studentData: Student[]) {
    return  this.http.post<{message: string, status: number, data: any[]}>(`${this.apiUrl3}/api/actions_add_2group`, studentData)
      .toPromise();
  }
  ...
  sendNewStudentDataForGroup(studentData: Student[]): void {
    ...
    this.subscriptionJoinGroup = this.http.post<{message: string, status: number, data: any[]}>(`${this.apiUrl3}/api/actions_add_2group`, studentData)
      .subscribe((data) => {
