...
export class PaymentsService {
  ...
  getBalanceAdvanced(postingInfo: AdvPostingInfo): AsyncSubject<any[]> {
    ...
    // const myQualificationList : StudentLedgerEntry[] = [] ;
    let query = `MATCH (f:Faculty)<-[:IN_FACULTY]-(p:Programme)<-[:A_STUDENT_OF]-(n:Student)<-[:POSTINGS_FOR]-(l:StudentLedgerEntry) where (n)-[:COMMENCED_STUDY]->(:Study{status:"Ongoing"})
    `;
