...
export class StudentTable2Component implements OnInit {
  ...
  displayedColumns = ['studentNo', 'title', 'lastName', 'firstName',  'middleName', 'gender' , 'department','phone', 'sType','level','dOB', 'address', 'state', 'religion', 'status','email','nin']; // , 'dOB',
...
loadFilterData2(option?: number): void {
    ...
    dataToUse.forEach((data: Student) => {
      ...
      {
        if (this.searchList[4].value.toUpperCase() === data.department?.toUpperCase())
        {statusOfCheckList.push(true)}
        else {statusOfCheckList.push(false)}
      }
  ...
  resetSearchObject(): void {
    this.searchList = [];
    const propertiesList = [ 'lastName', 'firstName',  'gender' ,  'level', 'department',  'status','faculty', 'studyStatus']
