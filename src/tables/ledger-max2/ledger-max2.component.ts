...
@Component({
  selector: 'app-ledger-max2',
  templateUrl: './ledger-max2.component.html',
  styleUrls: ['./ledger-max2.component.css']
})
export class LedgerMax2Component implements OnInit {
  // @Input()
  ...
  'details', 'studentNo', 'lastName', 'firstName', 'gender', 'level', 'department', 'actions']; // , 'dOB',
...
print(): void {
  ...
  applyFilter(event: Event & {target: HTMLInputElement}) {
    const {target} = event;
